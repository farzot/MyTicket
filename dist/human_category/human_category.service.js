...
let HumanCategoryService = class HumanCategoryService {
    ...
    async findOne(id) {
        ...
        if (!seat) {
            throw new common_1.NotFoundException(`humanyCategory with ID ${id} not found`);
        }
        return seat;
    }
    async updateHumanCategory(id, updateHumanCategoryDto) {
        try {
            const [numberOfAffectedRows, [updateHumanCategory]] = await this.humanCategoryRepo.update(updateHumanCategoryDto, {
                where: { id },
                returning: true,
            });
            if (numberOfAffectedRows === 0) {
                throw new common_1.NotFoundException(`HumanCategoryModule with ID ${id} not found`);
            }
    ...
    async remove(id) {
        try {
            ...
            throw new Error(`Error removing Human Category with ID ${id}: ${error.message}`);
        }
    }
};
...
exports.HumanCategoryService = HumanCategoryService = __decorate([
    ...
    __param(0, (0, sequelize_1.InjectModel)(human_category_model_1.Human_category)),
